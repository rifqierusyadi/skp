...
<div id="container">
	...
	<div id="body">
			...
			<div class="form-group <?= form_error('nip') ? 'has-error' : null; ?>">
				...
				<input name="nip" id="nip" type="text" value="<?= set_value('nip',$record->nip); ?>" class="form-control">
				<?php echo form_error('NIP', '<div class="help-block-">','</div>'); ?> 
			<br>
			<div class="form-group <?= form_error('password') ? 'has-error' : null; ?>">
				<label class="control-label">PASSWORD</label>
				<input name="password" id="password" type="text" value="<?= set_value('password',$record->password); ?>" class="form-control">
				<?php echo form_error('password','<div class="help-block-">','</div>'); ?> 
			<br>
			<div class="form-group <?= form_error('email') ? 'has-error' : null; ?>">
				<label class="control-label">EMAIL</label>
				<input name="email" id="email" type="email" value="<?= set_value('email',$record->email); ?>"class="form-control">
			...
			<div class="form-group <?= form_error('keterangan') ? 'has-error' : null; ?>">
				...
				<input name="keterangan" id="keterangan" type="text" value="<?= set_value('keterangan',$record->keterangan); ?>"class="form-control">
				<?php echo form_error('keterangan','<div class="help-block-">','</div>'); ?>
			<br>
			<p></p>
			<div class="form-group <?= form_error('active') ? 'has-error' : null; ?>">
			<label>STATUS</label>
			<select name="active" id="active" class="form-control"> 
				<option value=""> Pilih Salah Satu </option>					
				<option value="1"<?=set_select('active','1', $record->active == 1 ? TRUE : FALSE); ?>>Aktif</option>
				<option value="0"<?=set_select('active','0', $record->active == 0 ? TRUE : FALSE); ?>>Tidak Aktif</option>
			</select>
			<br>
